...
# valuable data.
def get_worldometers_data():
    f = open("txt_files/siteHTML.txt", "w")
...
def init_list():
    ...
    f = open("txt_files/listOfCountries.txt", "r")
    for line in f:
        temp = str(line.strip())
...
def create_countries(countryList):
    ...
    for line in f:
        if line.strip() == "<tr style=\"\">":
            ...
            update_country(countryList, valueDict)
...
def update_country(countryList, valueDict):
